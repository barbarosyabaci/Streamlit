...
if selected == "Geolocation Based RAN Infrastructure Planning":
    ...
    for i in range(len(df_points)):
        ...
        min_results_rohre.append(min_distance_2[0])
        min_results_trassen.append(min_distance_3[0])

    df_result["Point"] = points
    df_result["Site Name"] = df_tokyo_mongodb["Site Name"]
    df_result["Min_Distance_Own"] = min_results_own
    df_result["Min_Distance_Rohre"] = min_results_rohre
    df_result["Min_Distance_Trassen"] = min_results_trassen
...
if selected == "Database Management Tools":
    from pymongo.mongo_client import MongoClient
    from pymongo.server_api import ServerApi
    import pandas as pd
